...
def preprocess(rank, model, tokenizer, batch):
    ...
    claim_encoded = tokenizer.batch_encode_plus([batch[rank]['question_text']], padding='longest',  max_length=512, \
                                                    truncation=True, return_tensors='pt')
    claim_input_ids = claim_encoded.input_ids

    claim_input_ids_repeated = torch.repeat_interleave(claim_input_ids, len(batch[rank]['paragraphs']), dim=0)
    ...
    for i in range(0, len(paragraph_input_ids), args.shard_size):
        ...
        decoder_input_ids = claim_input_ids_repeated[i: i + args.shard_size].to(device=rank)
    ...
    row = {'question': batch[rank]['question'], 'answer': batch[rank]['answer'], 'candidates': reordered_candidates}
    return row
...
def main(args):
    ...
    #while True:
        ...
        output_path = f'{args.output_dir}/{args.dataset}/{args.data_split}_top1000/rerankinput_{args.rerank_type}/'#top2rerankt0ppgpt/' 
