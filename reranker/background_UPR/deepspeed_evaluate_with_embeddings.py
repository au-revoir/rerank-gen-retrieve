...
def inference(local_rank, model, question_tokenized, passages_tokenized):

    #print("local rank: ", local_rank,"encoder input: ", encoder_input_ids[0])
    question_input_ids, question_attn_mask = question_tokenized.input_ids.to(rank), question_tokenized.attention_mask.to(rank)
    passages_input_ids, passages_attn_mask = passages_tokenized.input_ids.to(rank), passages_tokenized.attention_mask.to(rank)
    with torch.no_grad():
        output_question = model.encoder(input_ids=question_input_ids, attention_mask=question_attn_mask, return_dict=True)
        output_passages = model.encoder(input_ids=passages_input_ids, attention_mask=passages_attn_mask, return_dict=True)

    question_embeddings = torch.mean(output_question.last_hidden_state, dim=1)
    passages_embeddings = torch.mean(output_passages.last_hidden_state, dim=1)

    #print("logits: ", logits.shape)
    output_sim_scores = []
    for i, j in zip(passages, passage_embeddings):
        output_sim_scores.append({'context': i, 'sim_score': util.pytorch_cos_sim(question_embeddings, j)})
    return output_sim_scores
...
def preprocess(rank, model, tokenizer, batch):
    ...
    question = batch[rank]['question']
    paragraphs = batch[rank]['paragraphs']
    ...
    #print("question_text: ", batch[rank]['question_text'])
    #print("paragraph text: ", batch[rank]['paragraphs'])
    #quit()

    question_tokenized = tokenizer.encode_plus(question, return_tensors="pt")

    sharded_sim_score_list = []
    #print("length paragraph input ids: ", len(paragraph_input_ids))
    for i in range(0, len(paragraphs), args.shard_size):
        paragraph_shard = paragraphs[i: i + args.shard_size].to(device=rank)
        paragraph_shard_tokenized = tokenizer.encode_plus(paragraph_shard, return_tensors="pt").to(device=rank)
        sharded_sim_score_list.extend(inference(rank, model, question_tokenized, paragraph_shard_tokenized))
    ...
    row = {'question': batch[rank]['question'], 'answer': batch[rank]['answer'], 'candidates': sharded_sim_score_list}
    return row
...
def main(args):
    ...
    "zero_optimization": {
        ...
        "stage3_param_persistence_thresh:old": 1 * model_hidden_size
    },
    ...
    #model = AutoModelForSeq2SeqLM.from_pretrained(args.model_name, cache_dir="/data/gg676/")
    model = T0(args.model_name, sync_gpus=True)
    #model = SentenceTransformer('sentence-transformers/sentence-t5-xxl', cache_folder='/data/local/gg676/')
    ...
    #prompt = "Explain?"
    provenance_data = load_json(args.data_path)[:10]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--verbalizer', type=str, default='Please write a question based on this passage.')
